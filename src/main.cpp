...
static int verthashMetal_thread(void *userdata)
{
    ...
    // Use verthash data from global info (already loaded)
...
int utf8_main(int argc, char *argv[])
{
    ...
    std::vector<mtldevice_t> mtldevices;

    if (metalDeviceCount > 0)
    {
        ...
        for (int i = 0; i < metalDeviceCount; i++)
        {
            ...
            {
                mtldevices.push_back(mtlDev);
                applog(LOG_INFO, "Metal Device %d: %s", i, mtlDev.deviceName);
                applog(LOG_INFO, "  Type: %s", mtlDev.isLowPower ? "Integrated GPU" : "Discrete GPU");
                applog(LOG_INFO, "  Max threads per group: %zu", mtlDev.maxThreadsPerThreadgroup);

                // Prefer Metal for discrete (non-low-power) GPUs
    ...
    {
        applog(LOG_INFO, "No Metal devices found");
    }
#endif
...
#ifdef HAVE_METAL
        // print Metal devices
        if (mtldevices.size() != 0)
        {
            puts("Metal devices:");
            for (size_t i = 0; i < mtldevices.size(); ++i)
            {
                printf("    [%zu] %s\n", i, mtldevices[i].deviceName);
                printf("        Type: %s\n", mtldevices[i].isLowPower ? "Integrated" : "Discrete");
                printf("        Max threads per group: %zu\n", mtldevices[i].maxThreadsPerThreadgroup);
            }
            puts("");
        }
        else
        {
            puts("Metal devices: None\n");
        }
#endif
...
#ifdef HAVE_METAL
    std::vector<mtlworker_t> mtlworkers;

    // Automatically configure Metal workers for discrete GPUs
    // Metal is preferred for discrete AMD GPUs on macOS for better performance
    for (size_t i = 0; i < mtldevices.size(); ++i)
    {
        // Only use Metal for discrete GPUs (non-low-power)
        // Integrated GPUs will continue using OpenCL for compatibility
        if (!mtldevices[i].isLowPower)
        {
            mtlworker_t mtlworker;
            mtlworker.mtldevice = mtldevices[i];
            mtlworker.workSize = 65536;  // Maximum workSize for discrete GPU performance
            mtlworker.batchTimeMs = vh::defaultBatchTimeMs;
            mtlworker.occupancyPct = vh::defaultOccupancyPct;
            mtlworker.gpuTemperatureLimit = vh::defaultGPUTemperatureLimit;
            mtlworker.deviceMonitor = vh::defaultDeviceMonitor;
            mtlworkers.push_back(mtlworker);

            applog(LOG_INFO, "Configured Metal worker for discrete GPU: %s", mtldevices[i].deviceName);
        }
    }
#endif
...
#ifdef HAVE_METAL
    opt_n_threads += mtlworkers.size();
#endif
    if (opt_n_threads > 0)
#ifdef HAVE_METAL
        applog(LOG_INFO, "Configured %llu(CL), %llu(CUDA), and %llu(Metal) workers", clworkers.size(), cuworkers.size(), mtlworkers.size());
#else
    ...
    opt_n_threads += mtlworkers.size();
    if (opt_n_threads > 0)
        applog(LOG_INFO, "Configured %llu(CL) and %llu(Metal) workers", clworkers.size(), mtlworkers.size());
#else
    if (opt_n_threads > 0)
    ...
    //-------------------------------------
    // Metal workers
    size_t mtlThreadOffset = clworkers.size();
#ifdef HAVE_CUDA
    mtlThreadOffset += cuworkers.size();
#endif

    for (size_t i = 0; i < mtlworkers.size(); i++)
    {
        mtlworkers[i].threadInfo = &thr_info[mtlThreadOffset + i];
        mtlworkers[i].threadInfo->id = (int)(mtlThreadOffset + i);
        mtlworkers[i].threadInfo->q = tq_new();
        if (!mtlworkers[i].threadInfo->q)
        {
            return 1;
        }

        if (unlikely(thrd_create(&mtlworkers[i].threadInfo->pth, verthashMetal_thread, &mtlworkers[i]) != thrd_success))
        {
            applog(LOG_ERR, "Metal thread %d create failed", (int)i);
            return 1;
        }
    }
#endif
